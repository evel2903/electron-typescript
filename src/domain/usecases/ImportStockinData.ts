  ...
  async execute(csvData: any[]): Promise<ImportResult> {
    try {
      ...
      for (const row of csvData) {
        try {
          if (!row.input_date || !row.slip_number || !row.location || !row.product_code || !row.staff_code || !row.shop_code) {
          ...
          stockinData.push({
            ...
            supplierCode: row.supplier_code ? String(row.supplier_code).trim() : undefined,
            supplierName: row.supplier_name ? String(row.supplier_name).trim() : undefined,
