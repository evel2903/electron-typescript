...
export class InventoryDataRepository implements IInventoryDataRepository {
  ...
  async bulkUpsert(inventoryData: Omit<InventoryData, 'id' | 'createdAt' | 'updatedAt'>[]): Promise<{ inserted: number; updated: number }> {
    let inserted = 0;
    let updated = 0;
    ...
    try {
      ...
      this.logger.info(`Inventory data bulk upsert completed: ${inserted} inserted, ${updated} updated`);
  ...
  private async findExistingRecord(inventory: Omit<InventoryData, 'id' | 'createdAt' | 'updatedAt'>): Promise<DatabaseInventoryData | null> {
    try {
