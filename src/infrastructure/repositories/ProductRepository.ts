...
export class ProductRepository implements IProductRepository {
    ...
    try {
      ...
      if (existingProduct) {
        await this.sqliteService.run(
          ...
          [product.productName, product.boxQuantity, product.supplierCode, product.janCode]
        );
      } else {
        await this.sqliteService.run(
          'INSERT INTO product (jan_code, product_name, box_quantity, supplier_code) VALUES (?, ?, ?, ?)',
          [product.janCode, product.productName, product.boxQuantity, product.supplierCode]
        );
      }
  ...
  async deleteProduct(janCode: string): Promise<boolean> {
    try {
      await this.sqliteService.run(
        'DELETE FROM product WHERE jan_code = ?',
        [janCode]
      );
      return true;
    } catch (error) {
  ...
  async bulkUpsert(products: Omit<Product, 'createdAt' | 'updatedAt'>[]): Promise<{ inserted: number; updated: number }> {
    let inserted = 0;
    let updated = 0;
