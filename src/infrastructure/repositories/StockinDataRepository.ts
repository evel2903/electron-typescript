...
export class StockinDataRepository implements IStockinDataRepository {
  ...
  async bulkUpsert(stockinData: Omit<StockinData, 'id' | 'createdAt' | 'updatedAt'>[]): Promise<{ inserted: number; updated: number }> {
    let inserted = 0;
    let updated = 0;
    ...
    try {
      ...
      this.logger.info(`Stockin data bulk upsert completed: ${inserted} inserted, ${updated} updated`);
  ...
  private async findExistingRecord(stockin: Omit<StockinData, 'id' | 'createdAt' | 'updatedAt'>): Promise<DatabaseStockinData | null> {
    try {
