...
export class DataSyncService {
    ...
    try {
      ...
      try {
        ...
        for (let i = 0; i < tablesToSync.length; i++) {
          ...
          try {
            const result = await this.syncTable(remoteDb, tableName, progressCallback, i, tablesToSync.length);
            results.push(result);
          } catch (error) {
      ...
      if (localTempDbPath && fs.existsSync(localTempDbPath)) {
        try {
          ...
          this.logger.warn(`Failed to clean up temporary database: ${localTempDbPath}`, error as Error);
        }
      }
    }
  ...
  private async pullDatabaseFromDevice(deviceId: string, remotePath: string): Promise<string> {
    ...
    try {
      ...
      const transferResult = await this.adbService.pullFile(deviceId, remotePath, localTempPath);
      
      if (!transferResult.success) {
        throw new Error(`Failed to pull database from device: ${transferResult.error || transferResult.message}`);
      }
  ...
  private async syncTable(
    ...
    try {
      ...
      const tableExists = remoteDb.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(tableName);
      ...
      for (let i = 0; i < records.length; i += batchSize) {
        ...
        if (progressCallback) {
          ...
          const overallProgress = Math.round(((currentTableIndex + tableProgress) / totalTables) * 100);
  ...
  private async processBatch(tableName: string, records: any[]): Promise<{ inserted: number; updated: number }> {
    let inserted = 0;
    let updated = 0;
