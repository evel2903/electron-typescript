...
export class FileParserService {
    ...
    private async parseExcel(filePath: string): Promise<FileParseResult> {
        try {
            ...
            this.logger.info(
                `Successfully parsed Excel file: ${data.length} data rows, detected type: ${detectedType}`,
            );
    ...
    private detectFileType(headers: string[]): ImportFileType | undefined {
        ...
        if (
            normalizedHeaders.some(
                (h) => h.includes('shop_code') || h.includes('shop') || h.includes('location_code'),
            ) &&
            normalizedHeaders.some(
                (h) =>
                    h.includes('shop_name') || h.includes('location_name') || h.includes('store'),
            )
        ) {
            return 'location';
        }
        ...
        if (
            normalizedHeaders.some(
                (h) =>
                    h.includes('staff_code') ||
                    h.includes('employee_code') ||
                    h.includes('emp_code'),
            ) &&
            normalizedHeaders.some(
                (h) =>
                    h.includes('staff_name') ||
                    h.includes('employee_name') ||
                    h.includes('emp_name'),
            )
        ) {
            return 'staff';
        ...
        if (
            normalizedHeaders.some(
                (h) => h.includes('supplier_code') || h.includes('vendor_code'),
            ) &&
